...
void setup() {
  ...
  oregon.start(); 
  oregon.receiver_dump = 0; //true - Включает "осциллограф"
}
...
void loop() {
  ...
  if (oregon.captured)  {
    ...
    if ((oregon.sens_type == THGN132 || (oregon.sens_type & 0x0FFF) == RTGN318 || oregon.sens_type == THGR810 || oregon.sens_type == THN132) && oregon.crc_c){
      Serial.print("\t");
      if (oregon.sens_type == THGN132 || (oregon.sens_type & 0x0FFF) == RTGN318 || oregon.sens_type == THGR810)  for (int q = 0;q < PACKET_LENGTH - 1; q++) Serial.print(oregon.packet[q], HEX);
  ...
  if (oregon.sens_type == WGR800 && oregon.crc_c){
      Serial.print("\t");
      for (int q = 0; q < PACKET_LENGTH - 1; q++) Serial.print(oregon.packet[q], HEX);
      float wnddata;
      Serial.print(" TYPE: ");
      Serial.print("WGR800");
      Serial.print(" AVG WS: ");
